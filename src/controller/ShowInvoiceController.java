...
img.setCellFactory(new Callback<TableColumn<Facture, Image>, TableCell<Facture, Image>>(){
    ...
    public void afficheinvoice() {
        try {
            ...
            rs = stmt.executeQuery("SELECT image_name,libelle,prixProduit,commande_tot,qte_prod,prixtotal,meth_livraison,paiement FROM commande,livraison,utilisateur,produit,details_commande where livraison.id_commande = commande.id_commande and details_commande.id_commande = commande.id_commande and details_commande.id_prod = produit.id and livraison.id_user = utilisateur.id and livraison.id_commande = "+id_comm);
            System.out.println("SELECT image_name,libelle,prixProduit,commande_tot,qte_prod,prixtotal,meth_livraison,paiement FROM commande,livraison,utilisateur,produit,details_commande where livraison.id_commande = commande.id_commande and details_commande.id_commande = commande.id_commande and details_commande.id_prod = produit.id and livraison.id_user = utilisateur.id and livraison.id_commande = "+id_comm);
//ss
            
            while (rs.next()) {
                Imagess = rs.getString("image_name");

                String nom_produit = rs.getString("libelle");
                int prix_prod = rs.getInt("prixProduit");
